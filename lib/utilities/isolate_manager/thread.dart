...
class _Worker with ThreadFlags implements Thread {
  ...
  Stream<Result> work({@required Task task}) async* {
    if (!isClosed) {
      ...
      _sendPort.send(IsolateBundle(port: receivePort.sendPort, function: task.function, bundle: task.bundle, timeout: task.timeout));
      ...
      taskId = '';
      isBusy = false;
      yield resultFromIsolate is ErrorResult ? Result.error(resultFromIsolate.asError.error) : Result.value(resultFromIsolate.asValue.value);
    }
  }
