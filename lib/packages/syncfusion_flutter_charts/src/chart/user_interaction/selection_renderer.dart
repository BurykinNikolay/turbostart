...
class _SelectionRenderer {
  ...
  void selectionIndex(int _pointIndex, int _seriesIndex,
    ...
    if (_validIndex(_pointIndex, _seriesIndex, chart)) {
      series = chart.series[_seriesIndex];
      series.selectionSettings._selectionRenderer.selectionType =
          _selectionType;
      bool select = false;
      if (series._seriesType == 'line' ||
        ...
        if (series.selectionSettings.enable) {
          series.selectionSettings._selectionRenderer.cartesianPointIndex =
              _pointIndex;
          series.selectionSettings._selectionRenderer.cartesianSeriesIndex =
              _seriesIndex;
          select = series.selectionSettings._selectionRenderer
              .isCartesianSelection(chart, series, _pointIndex, _seriesIndex);
        }
      } else {
        ...
        if (series._seriesType.contains('area') ||
          ...
          series.selectionSettings._selectionRenderer.seriesIndex =
              _seriesIndex;
          series.selectionSettings._selectionRenderer.pointIndex = _pointIndex;
        }
        select = series.selectionSettings._selectionRenderer
...
*/
  ...
  Paint getDefaultFillColor(
    ...
    if (series is CandleSeries) {
      if (segment is CandleSegment && segment.isSolid) {
...
// To check that touch point is lies in segment
  ...
  bool _isSeriesContainsPoint(dynamic series, Offset position) {
    ...
    if (dataPointIndex != null && series.segments.isNotEmpty) {
      ...
      if (series._seriesType.contains('hilo') ||
        ...
        if (dataPointIndex < series._dataPoints.indexOf(dataPoint))
          ...
          series._dataPoints.indexOf(dataPoint) - 1 < 0
              ...
              : nearestDataPointIndex =
                  series._dataPoints.indexOf(dataPoint) - 1;
        } else
          nearestDataPointIndex = series._dataPoints.indexOf(dataPoint);
      }
    }
