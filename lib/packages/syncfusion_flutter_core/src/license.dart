  ...
  static KeyInfo _decrypt(String encryptedKey) {
    try {
      ...
      for (int i = 0, k = 0; i < encryptedKey.length; i++) {
        k = k < _key.length ? k++ : 0;
        final List<int> encryptedKeyValue = encodeUtf32(encryptedKey.substring(i, i + 1));
        final List<int> encryptedBaseKeyValue = encodeUtf32(_key.substring(k, k + 1));
        final int x = encryptedKeyValue[encryptedKeyValue.length - 1] ^ encryptedBaseKeyValue[encryptedBaseKeyValue.length - 1];
      ...
      final String exactFormat = year +
          ...
          ' ' +
          (dateFormat.split(' ')[2] == 'AM' ? (hours + ':' + minutes + ':' + seconds) : ((int.parse(hours) + 12).toString() + ':' + minutes + ':' + seconds));
      final DateTime expiryDate = DateTime.parse(exactFormat);
      final String mailID = decryptedText.split(';')[3];
      return KeyInfo(version: version, platform: platform, expiryDate: expiryDate, mailID: mailID);
    } catch (e) {
      // ignore: avoid_print
